...
The [test suite](/tests/Tests.hs) contain MANY more examples of the types of expressions the Herbie plugin can analyze.
...
Fortunately, we can still analyze the non-recursive subexpressions of a recursive expression.
So in the code:
```
...
The simplest source is a sqlite3 database located at `$HOME/.Herbie/Herbie.db`.
This database contains about 400 commonly seen expressions and their replacements.

The more important source is the [Herbie program](http://herbie.uwplse.org/).
Herbie is a recent research project on using probabilistic tree searches to find numerically stable expressions.
...
If all the expressions in the code are already in the database,
